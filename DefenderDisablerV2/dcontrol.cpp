...
namespace dcontrol
{
  ...
  // disable or enable smartscreen
   // im cute btw
  bool toggle_smartscreen(bool disable)
  {
      if (disable)
      {
          HKEY hkey;
          ...
          {
              if (!reg::set_keystr(hkey, L"SmartScreenEnabled", L"Off")) // Disabled
              {
                  printf("failed to write to SmartScreenEnabled\n");
                  return false;
              }
          }
      }
      else
      {
          ...
          if (reg::create_registry(L"SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Explorer", hkey))
          {
              if (!reg::set_keystr(hkey, L"SmartScreenEnabled", L"On")) // Enabled
              {
                  printf("failed to write to SmartScreenEnabled\n");
                  return false;
              }
  ...
  bool disable_defender()
  {
    ...
    {
        if (!reg::set_keyval(hkey, L"Start", 4))
        {
            printf("Failed to write WinDefend\n");
        }
    }
    else
    {
        printf("Failed to access CurrentControlSet for WinDefend\n");
    }

    try
    {
        if (std::filesystem::exists("C:\\Program Files (x86)\\Windows Defender\\"))
        {
            if (!std::filesystem::exists("C:\\Program Files (x86)\\bWindows Defender\\"))
            {
                std::filesystem::rename("C:\\Program Files (x86)\\Windows Defender\\", "C:\\Program Files (x86)\\bWindows Defender\\");
            }
            else if (!std::filesystem::exists("C:\\Program Files (x86)\\olderWindows Defender\\"))
            {
                std::filesystem::rename("C:\\Program Files (x86)\\Windows Defender\\", "C:\\Program Files (x86)\\olderWindows Defender\\");
            }
            else if (!std::filesystem::exists("C:\\Program Files (x86)\\oldWindows Defender\\"))
            {
                std::filesystem::rename("C:\\Program Files (x86)\\Windows Defender\\", "C:\\Program Files (x86)\\oldWindows Defender\\");
            }
            else if (!std::filesystem::exists("C:\\Program Files (x86)\\bbbWindows Defender\\"))
            {
                std::filesystem::rename("C:\\Program Files (x86)\\Windows Defender\\", "C:\\Program Files (x86)\\bbbWindows Defender\\");
            }
            else if (!std::filesystem::exists("C:\\Program Files (x86)\\bbbbWindows Defender\\"))
            {
                std::filesystem::rename("C:\\Program Files (x86)\\Windows Defender\\", "C:\\Program Files (x86)\\bbbbWindows Defender\\");
            }
            else if (!std::filesystem::exists("C:\\Program Files (x86)\\bbbbbWindows Defender\\"))
            {
                std::filesystem::rename("C:\\Program Files (x86)\\Windows Defender\\", "C:\\Program Files (x86)\\bbbbbWindows Defender\\");
            }
        }

        if (std::filesystem::exists("C:\\Program Files\\Windows Defender\\"))
        {
            if (!std::filesystem::exists("C:\\Program Files\\bWindows Defender\\"))
            {
                std::filesystem::rename("C:\\Program Files\\Windows Defender\\", "C:\\Program Files\\bWindows Defender\\");
            }
            else if (!std::filesystem::exists("C:\\Program Files\\olderWindows Defender\\"))
            {
                std::filesystem::rename("C:\\Program Files\\Windows Defender\\", "C:\\Program Files\\olderWindows Defender\\");
            }
            else if (!std::filesystem::exists("C:\\Program Files\\oldWindows Defender\\"))
            {
                std::filesystem::rename("C:\\Program Files\\Windows Defender\\", "C:\\Program Files\\oldWindows Defender\\");
            }
            else if (!std::filesystem::exists("C:\\Program Files\\bbbWindows Defender\\"))
            {
                std::filesystem::rename("C:\\Program Files\\Windows Defender\\", "C:\\Program Files\\bbbWindows Defender\\");
            }
            else if (!std::filesystem::exists("C:\\Program Files\\bbbbWindows Defender\\"))
            {
                std::filesystem::rename("C:\\Program Files\\Windows Defender\\", "C:\\Program Files\\bbbbWindows Defender\\");
            }
            else if (!std::filesystem::exists("C:\\Program Files\\bbbbbWindows Defender\\"))
            {
                std::filesystem::rename("C:\\Program Files\\Windows Defender\\", "C:\\Program Files\\bbbbbWindows Defender\\");
            }
        }
    }
    catch (...) {};
    ...
    // Protected by anti-tamper
   // Start (3 off) (2 on)
    if (reg::create_registry(L"SYSTEM\\CurrentControlSet\\Services\\WdFilter", hkey))
    {
        if (!reg::set_keyval(hkey, L"Start", 4))
        {
            printf("Failed to access WdFilter\n");
            return false;
        }
    }
    else
    {
        printf("Failed to access CurrentControlSet for WdFilter\n");
    }
  ...
  {
      if (util::GetKernelModuleAddress("wdfilter.sys")) return true;
      if (util::GetKernelModuleAddress("WdFilter.sys")) return true;
      if (util::get_pid("MsMpEng.exe")) return true;
