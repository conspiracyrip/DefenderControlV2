...
namespace reg
{
  ...
  bool set_keystr(HKEY& hkey, const wchar_t* value_name, const wchar_t* value)
  {
      DWORD cbData = (wcslen(value) + 1) * sizeof(TCHAR); //

      auto ret = RegSetValueExW(hkey, value_name, 0, REG_SZ, (LPBYTE)value, cbData);
